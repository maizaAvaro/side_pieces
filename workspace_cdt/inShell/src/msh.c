...
/*static const int HISTORY_MAX_SIZE = 11;
...
int checkBuiltInCommands()
{
       ...
       /* if (strcmp("in", commandArgv[0]) == 0) {
		...
		if (commandArgv[1] == NULL)
                ...
                if (job->status == SUSPENDED || job->status == WAITING_INPUT){
                        fprintf("bringing program from background to run: ");
                		puts(commandArgv[1]);
	...
	if(strcmp("history", commandArgv[0]) == 0)
	{
		...
		//for(index = 0; index < history.size(); index++){
			//fprintf(stdin, "Value of history: %c\n", history[index]);
		//}
		return 1;
...
void signalHandler_child(int p)
{
        pid_t pid;
        int terminationStatus;
        // previously had WAIT_ANY in place of -1...
